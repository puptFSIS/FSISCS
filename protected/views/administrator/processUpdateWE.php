<?php
	...
	if($d1==null || $d2==null) {
		header("location:index.php?r=administrator/Uwe&msg=Please fill up all the required fields&msgType=err&pos=$pos&fm=$fm&fd=$fd&fy=$fy&tm=$tm&td=$td&ty=$ty&com=$com");
	} else {
	if(isset($_GET['mode'])) {
		$mode = $_GET['mode'];
		if($mode==1) {
			...
			$result=mysqli_query($conn,$sql);		
				if($result) {
					header("Location: index.php?r=administrator/we&msg=Your work experience has been updated successfully!");
				} else {
