...
<!-- Page title -->
<title>Faculty | New Faculty</title>
<!-- Page icon -->
...
<h2 class="underlined-header">New Faculty Members</h2>
<?php
	if(isset($_GET['msg'])) {
	$msg = $_GET['msg'];
...
?>
<h4 class="underlined-header"><strong>Select a Semester</strong></h4>
<form name="frmSched" method = "post" action = "index.php?r=administrator/NewFaculty">
...
<br/>
<h4 class="underlined-header"><strong>Select a School Year</strong></h4>
...
<?php
	if(isset($_POST['sem'])and isset($_POST['sy'])){
		...
		$sy = $_POST['sy'];
		echo'
		<a href="index.php?r=administrator/AddNewFaculty&sem='.$sem.'&sy='.$sy.'" class="btn btn-mini"><button>Add</button></a>
		...
		<tbody>';
		$sql = "SELECT * FROM tbl_masterlist WHERE Status = 'NF' and sem = '$sem' and schoolYear = '$sy' order by FName ASC";
		...
		';
	} elseif(isset($_GET['sem'])and isset($_GET['sy'])){
		$sem = $_GET['sem'];
		$sy = $_GET['sy'];
		echo'
		<a href="index.php?r=administrator/AddNewFaculty&sem='.$sem.'&sy='.$sy.'" class="btn btn-mini"><button>Add</button></a>
