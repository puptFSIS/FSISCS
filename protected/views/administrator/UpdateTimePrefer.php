...
.cssWLGradientIMG
{BACKGROUND-IMAGE: url(images/hd_tm1.jpg);BACKGROUND-REPEAT:repeat-x;top:0;height:105px;}</style>
...
<form id="annc" name="annc" action="index.php?r=administrator/TimePreferProcess&sem=<?php echo $_GET['sem']?>&sy=<?php echo $_GET['sy']?>&timeID=<?php echo $timeID;?>" method="post">
...
<select name="profName" style="width: 470px; margin-top: -28px; margin-left: 15%;">
	<?php
		...
		$sql="SELECT * FROM tbl_evaluationfaculty where status = 'Active' AND (evalRoles = 'Professor' OR evalRoles = 'Administrator') order by LName ASC";
		$result = mysqli_query($conn,$sql);
		while($row = mysqli_fetch_array($result)) 
		{
			echo '
				<option value="'. $row['FCode'] .'"> '. strtoUpper($row['LName']) .', '. strtoUpper($row['FName']) .'</option>
			';
		}
	?>
</select>
...
<center><p><input type="submit" value="Save" /> <a href="index.php?r=administrator/TimePrefer&sem=<?php echo $_GET['sem']?>&sy=<?php echo $_GET['sy']?>" class="btn btn-primary">Cancel</a></p></center>
</form>
</section>
<!-- End - Video -HTML5 -->
<br/>
...
<script type="text/javascript">
	...
	if(flashdata==1){
		Swal.fire({
			icon:'error',
			title:'Ooops!',
			text:'Fill Out the Given Fields!'
		})
	}
