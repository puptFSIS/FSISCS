...
<br/>
    ...
    <!-- Buttons -->
    <a href="index.php?r=administrator/IPCRaddrow<?php echo'&m='.$m.'&y='.$y.'';?>">
        <button style="width: 80px;">Add Row</button>
    </a>
...
<thead>
    <tr>
        ...
        <th width="2%"><h5 align="center"></h5></th>
...
<tbody >
<?php 
        // Query to outPut the OutPut and Success Indicators
        $sql = "SELECT * FROM tbl_ipcr1 WHERE part='sp' AND deleted_on IS NULL AND year = '$y'";
...
<?php
        ...
        $sql = "SELECT * FROM tbl_ipcr1 WHERE part='cf' AND deleted_on IS NULL AND year = '$y'";
        $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_array($query)) :
        $id = $row['id'];
        $ifRequired = $row['if_required'];
...
<?php
        ...
        $sql = "SELECT * FROM tbl_ipcr1 WHERE part='sf' AND deleted_on IS NULL AND year = '$y'";
        $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_array($query)) :  
        $id = $row['id'];
        $ifRequired = $row['if_required'];
...
<center><a href="index.php?r=administrator/IPCRtagvisible<?php echo'&m='.$m.'&y='.$y.'';?>"><button style="width:120px">Tag as Visible</button></a></center>
            ...
            <!-- for clear deadline -->
            <?php if(isset($_GET['d'])) : ?>
                <div class="flash-data-clear" data-flashdata4="<?= $_GET['d']; ?>"></div>
            <?php endif; ?>
                ...
                <script>
                    ...
                    const flashdata4 = $('.flash-data-clear').data('flashdata4')
                    if (flashdata4) {
                        Swal.fire(
