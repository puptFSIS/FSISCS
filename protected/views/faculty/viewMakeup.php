...
<!-- Page title -->
<title>Makeup class module</title>
<!-- Page icon -->
...
<section>
	...
	<br><br><br><br>
		<table>
			...
			<?php
			$sql = mysqli_query($conn,"Select * from tbl_make_up where FCode = '$FCode' order by make_up_id DESC");
			while($row=mysqli_fetch_assoc($sql)){
			 $date = date('F j, Y',strtotime($row['date_makeup']));
			 $time = $row['time'];   
			echo '
			<tr>
				<td>'.$date.'</td>
				...
				elseif($row['patch_status']==1){
					echo '<td><span style="color:green;">Approved.</span></td>';
				}
				elseif($row['patch_status']==2){
					echo '<td><span style="color:red;">Rejected.</span></td>';
				}
				elseif($row['patch_status']==3){
					echo '<td><span style="color:blue;">Checked.</span></td>';
				}
			...
			<tr>
				<td align="right" colspan=5>
					<a href="index.php?r=faculty/applyMakeup"><button>Back</button></a>
				</td>
			</tr>
...
<li>
<a>Make-up class</a>
<span class=widget-hint><a href='index.php?r=faculty/applyMakeup'>Apply</a></span>
</li>
<li>
<a>Teaching Load</a>
