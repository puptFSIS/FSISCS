...
$sql = "SELECT * FROM tbl_bridge_subject_course WHERE BridgeSubjectID = ".$BridgeSubjectID;
$result = mysqli_query($conn,$sql);
...
<form id="annc" name="annc" action="index.php?r=administrator/ProcessEditBridgeSubject" method="post">
    ...
    <p style="margin-bottom: 9px;">*Professor:
        <select name="FCode" style="width: 470px; margin-top: -28px; margin-left: 15%;">
        <?php
        $sql = "SELECT * FROM tbl_evaluationfaculty ORDER BY LName";
        $result = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_array($result)) :
        ?>
            <?php if($row['FCode'] == $FCode) : ?>
                <option value="<?php echo $row['FCode'];?>" selected><?php echo $row['LName'].' '.$row['FName'];?></option>
            <?php endif; ?>

            <option value="<?php echo $row['FCode'];?>"><?php echo $row['LName'].' '.$row['FName'];?></option>
        <?php endwhile; ?>
        </select>
    </p>
    ...
    while($row = mysqli_fetch_array($result)) :
    ?>
        <?php if(in_array($row['course'], $courses)) : ?>
            <input name="course_selected[]" type="checkbox" value="<?php echo $row['course']; ?>" checked/><?php echo $row['course_code']; ?><br>
        <?php else : ?>
            <input name="course_selected[]" type="checkbox" value="<?php echo $row['course']; ?>"/><?php echo $row['course_code']; ?><br>
        <?php endif; ?>
    <?php endwhile; ?>
