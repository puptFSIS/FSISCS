...
<!-- Page title -->
<title>Faculty | Security Guards</title>
<!-- Page icon -->
...
<h2 class="underlined-header">Security Guards</h2>
<?php
	if(isset($_GET['msg'])) {
	$msg = $_GET['msg'];
...
?>
<form name="frmSched" method = "post" action = "index.php?r=administrator/SecurityGuard">
...
<?php
	if(isset($_POST['sy'])){
		$sy = $_POST['sy'];
		echo'
		<a href="index.php?r=administrator/AddSecurityGuard&sy='.$sy.'" class="btn btn-mini"><button>Add</button></a>
		...
		<tbody>';
		$sql = "SELECT * FROM tbl_masterlist WHERE Status = 'SG' and schoolYear = '$sy' order by FName ASC";
