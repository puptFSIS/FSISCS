...
/**
 * This is the model class for table "tbl_ipcr1".
 *
 * The followings are the available columns in table 'tbl_ipcr1':
 * @property integer $id
 ...
 */ 
class TblIpcr1 extends CActiveRecord
{
	/**
	 ...
	 * @return TblIpcr1 the static model class
	 */
	...
	public function getIPCR1datasp($y,$fcode)
	{
	...
	public function getIPCR1datacf($y,$fcode)
	{
		$sql= "SELECT tbl_ipcr1.*,tbl_ipcraccomp.* FROM tbl_ipcr1 LEFT JOIN tbl_ipcraccomp ON tbl_ipcraccomp.id_ipcr1 = tbl_ipcr1.id AND tbl_ipcraccomp.FCode = :fcode WHERE tbl_ipcr1.part = :cf AND tbl_ipcr1.year = :year AND tbl_ipcr1.deleted_on IS NULL ORDER BY tbl_ipcr1.id, tbl_ipcraccomp.id_ipcr1 ASC";
		$row = Yii::app()->db->createCommand($sql)
		->bindValue(':fcode', $fcode)
		->bindValue(':cf', 'cf')
		->bindValue(':year', $y)
	...
	public function getIPCR1datasf($y,$fcode)
	{
		$sql= "SELECT tbl_ipcr1.*,tbl_ipcraccomp.* FROM tbl_ipcr1 LEFT JOIN tbl_ipcraccomp ON tbl_ipcraccomp.id_ipcr1 = tbl_ipcr1.id AND tbl_ipcraccomp.FCode = :fcode WHERE tbl_ipcr1.part = :sf AND tbl_ipcr1.year = :year AND tbl_ipcr1.deleted_on IS NULL ORDER BY tbl_ipcr1.id, tbl_ipcraccomp.id_ipcr1 ASC";
		$row = Yii::app()->db->createCommand($sql)
		->bindValue(':fcode', $fcode)
		->bindValue(':sf', 'sf')
		->bindValue(':year', $y)
