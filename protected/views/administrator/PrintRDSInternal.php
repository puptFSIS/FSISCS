...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->Cell(0,3,'Room Daily Schedule',0,0,'C');
		$this->Ln();
		$this->Ln();
		...
		*/
		...
		if($cF == 0 or ($cF > 0 and $codeF2 == ""))
		{
			...
			else if($codeF<>"")
			{
				...
				$y=$this->GetY();
				$this->SetFillColor(260,260,260);
				$this->MultiCell($w[5],10.66, "$courseF\n$codeF\n$profF",1,'C',$fill);
				$this->setXY($x+$x-185,$y);
			}
			else
			{
	...
	function Footer()
{
	...
	*/
	...
	$sql = "SELECT * FROM tbl_room order by roomName ASC";
	$result = mysqli_query($conn,$sql);
	while($row = mysqli_fetch_array($result))
	{
		$pdf->AddPage('L');
		$pdf->table($row['roomName']);
	}
	$pdf->Output();
?>