...
class PDF extends FPDF
{
	...
	function table($cYear)
	{
		...
		$header = array('SUBJECT CODE', 'SUBJECT DESCRIPTION', 'LAB', 'LEC', 'UNITS');
		$w = array(50, 200, 20, 20, 20);
		...
		{ 
			...
			$this->Cell($w[0],10, '  ' . $row['SubjCode'],1,0,'L',$fill);
			$this->Cell($w[1],10, '  ' . $row['SubjDescription'],1,0,'L',$fill);
			$this->Cell($w[2],10, '' . $row['HoursLab'],1,0,'C',$fill);
			$this->Cell($w[3],10, '' . $row['HoursLec'],1,0,'C',$fill);
			$this->Cell($w[4],10, '' . $row['Units'],1,0,'C',$fill);
			$this->Ln();
