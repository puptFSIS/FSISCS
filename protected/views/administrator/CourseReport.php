...
class PDF extends FPDF
{
	...
	function table($cYear)
	{
		...
		$header = array('COURSE CODE', 'COURSE DESCRIPTION', 'STATUS', 'YEARS');
		$w = array(50, 210, 30, 20);
		...
		{ 
			...
			$this->Cell($w[0],10, '  ' . $row['course_code'],1,0,'L',$fill);
			$this->Cell($w[1],10, '' . $row['course_desc'],1,0,'L',$fill);
			$this->Cell($w[2],10, '' . $row['Status'],1,0,'C',$fill);
			$this->Cell($w[3],10, '' . $row['NoOfYears'],1,0,'C',$fill);
			$this->Ln();
