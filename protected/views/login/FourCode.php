...
<!-- Video - HTML5 -->
<section>
	<form action="index.php?r=login/ConfirmCode" id=dummy method=POST>
    
		<p>
		<label for=dummy0>Please check your email for a message with your code. Your code is 4 numbers long.</label>
		<input style="display:inline-block;" name='code' type=text oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*?)\..*/g, '$1');" style="width: 200px;" maxlength="4" required /><label >We sent your code to: <?php echo $email ?></label>
		</p>
        
		<p class="click">
		<input type=submit style="width:90px;" value='Continue' />
		</p>

		<input type="hidden" name="fcode" value = "<?php echo $fcode ?>">

		<a style="color: #FA4202;" href="index.php?r=login/SendAgain&fcode=<?php echo $fcode ?>">Didn't get the code?</a>
