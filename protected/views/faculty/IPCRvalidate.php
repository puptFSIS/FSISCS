<?php 
    ...
    $sql3 = "SELECT dline_date FROM tbl_ipcrstatus WHERE month = '$m' AND year = '$y'"; //Querying the deadline date
    ...
    $status = $row['status'];
        ...
        while ($row= mysqli_fetch_array($query))
        {
            ...
            if($count1 == 0)
            {
                header('Location: index.php?r=faculty/IPCRfaculty&m='.$m.'&y='.$y.'&mess=3');
            } else if ($count1 > 0) 
            {
                if($IsAvailable == "Not Available")
                {
                    header('Location: index.php?r=faculty/IPCRfaculty&m='.$m.'&y='.$y.'&mess=2');
                } else if ($IsAvailable == "Available") 
                {
                    if($count_row == NULL)
                    {
                        ...
                        } else if($deadline != NULL) //If Dealine is Not null or it was set then continue,
                        {
                            ...
                            if($deadline > $today) //If deadline is present, system will check if the the date exceed or not.
                            {
                                ...
                                } else if($count > 0)
                                {
                                 ...
                                 header('Location: index.php?r=faculty/IPCRfaculty&m='.$m.'&y='.$y.'&mess=1');
                            }
                        }
                    } elseif($count_row > 0) {///////////////
                        if($status == NULL)
                        {
                            ...
                            header('Location: index.php?r=faculty/IPCRfaculty&m='.$m.'&y='.$y.'&mess=4');
                        } elseif($status == "Pending") {
                            header('Location: index.php?r=faculty/IPCRfaculty&m='.$m.'&y='.$y.'&mess=5');
                        } elseif($status == "Approved") {
                            header('Location: index.php?r=faculty/IPCRfaculty&m='.$m.'&y='.$y.'&mess=6');
                        }
                    }
                }
            }
