...
<form id="annc" name="annc" action="index.php?r=faculty/TimePreferProcessUPDATE&sem=<?php echo $_GET['sem']?>&sy=<?php echo $_GET['sy']?>&timeID=<?php echo $timeID; ?>" method="post">
...
$profnow = $_SESSION['FCode'];
		...
		$sql="SELECT * FROM tbl_evaluationfaculty where FCode = '$profnow' AND  status = 'Active' AND (evalRoles = 'Professor' OR evalRoles = 'Administrator') order by LName ASC";
		$result = mysqli_query($conn, $sql);
		while($row = mysqli_fetch_array($result)) 
		{
			echo '
...
<center><p><input type="submit" value="Save" /> <a href="index.php?r=faculty/TimePrefer&sem=<?php echo $_GET['sem']?>&sy=<?php echo $_GET['sy']?>" class="btn btn-primary">Cancel</a></p></center>
</form>
</section>
<!-- End - Video -HTML5 -->
<br/>
