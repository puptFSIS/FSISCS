...
<?php 
    ...
    $dline_orig = $row['dline_date'];

    $query = "SELECT status,dline_date FROM tbl_ipcrstatus WHERE month = '$m' AND year = '$y' AND fcode = '$fcode'";
...
<br/>
<div id="main">
    <h2 id="title" >Create IPCR: <strong>January to June <?php echo '('.$y.')';?> - Deadline: <?php if($dline == NULL) { echo 'not set'; } else { if($dline > $dline_orig) { echo 'Extended until <u>'.$dline.'</u>.'; } else { echo $dline; } }?></strong></h2>
    <hr id="line">
    <!-- Deadline -->
    ...
    <tbody >
                ...
                <?php foreach($infosp as $rowsp): ?>
                    <tr>
                        ...
                        <?php if($rowsp['accomplishment'] == NULL): ?>
                            <?php if($status == "Pending" || $status == NULL): ?>
                                <td id="accomp" name="accomp" style="text-align: left;"><a href="#">Sync your Accomplishment with eQAR</a></td>
                            <?php else: ?>
                                <td id="accomp" name="accomp" style="text-align: left;"><?= $rowsp['accomplishment'] ?></td>
                            <?php endif; ?>
                        <?php elseif($rowsp['accomplishment'] != NULL): ?>
                            <td id="accomp" name="accomp" style="text-align: left;"><?= $rowsp['accomplishment'] ?></td>
                        <?php endif; ?>
