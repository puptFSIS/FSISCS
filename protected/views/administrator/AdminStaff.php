...
<!-- Page title -->
<title>Faculty | Administrative Staff</title>
<!-- Page icon -->
...
<h2 class="underlined-header">Administrative Staff</h2>
<?php
	if(isset($_GET['msg'])) {
	$msg = $_GET['msg'];
...
<form name="frmSched" method = "post" action = "index.php?r=administrator/AdminStaff">
	...
	<select name = "sy" style="width: 30%;">
		<?php
			...
			$sql = "SELECT DISTINCT schoolYear FROM tbl_curriculum order by schoolYear DESC";
...
<?php
	if(isset($_POST['sy'])){
		$sy = $_POST['sy'];
		echo'
		<a href="index.php?r=administrator/AddAdminStaff&sy='.$sy.'" class="btn btn-mini"><button>Add</button></a>
		...
		<tbody>';
		$sql = "SELECT * FROM tbl_masterlist WHERE Status = 'AS' and schoolYear = '$sy' order by FName ASC";
