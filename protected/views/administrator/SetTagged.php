...
<?php
		...
		$scode = $_GET['scode'];
		$sql2="SELECT DISTINCT scode, stitle FROM tbl_subjpreferences where schedID = '$schedID' AND scode = '$scode'";
		...
		echo '<h2 >Set Schedule <br/>';
		
		echo '<br/><h3 class = underlined-header>'. $_GET['scode'] . ': <br/> ' . $stitle . '</h3>';
...
<form id="annc" name="annc" action="index.php?r=administrator/processSetTagged&schedID=<?php echo $_GET['schedID'];?>&scode=<?php echo $_GET['scode'];?>&sem=<?php echo $_GET['sem'];?>&sy=<?php echo $_GET['sy'];?>" method="post">
...
<select name="sday" style="width: 470px; margin-top: -28px; margin-left: 15%;">
	<?php
		...
		$sql2="SELECT * FROM tbl_subjprefences where schedID = '$schedID' and  scode = '$scode'";
		$result2 = mysqli_query($conn,$sql2);
/*
		while($row2 = mysqli_fetch_array($result2))
		{
...
*/		
		if($rm <> "")
		{
			echo '<option value="'. $rm .'">'. to12Hr($rm) .'</option>';
		}
		else
		{
			echo'
...
<select name="profName" style="width: 470px; margin-top: -28px; margin-left: 15%;">
	<?php
		...
		$prof = "";
		$blank = "";
		$sql2="SELECT * FROM tbl_subjpreferences where schedID = '$schedID' and  scode = '$scode'";
		$result2 = mysqli_query($conn,$sql2);
   		while($row2 = mysqli_fetch_array($result2))
		{
			$prof = $row2['sprof'];
		...
		$sql="SELECT * FROM tbl_evaluationfaculty where status = 'Active' AND evalRoles = 'Professor' order by LName ASC";
		$result = mysqli_query($conn,$sql);
		while($row = mysqli_fetch_array($result)) 
		{
			echo '
