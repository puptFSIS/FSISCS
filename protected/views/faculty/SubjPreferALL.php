...
<br />
	<form name="frmSched" method = "post" action = "index.php?r=faculty/SubjPreferALL">
...
<tbody>
					...
					<?php						
						...
						if(isset($_POST['sem']) and isset($_POST['sy'])) 
						{
							$csem = $_POST['sem'];
							$sy = $_POST['sy'];
							$sqlo = "SELECT * from tbl_evaluationfaculty where FCode = '$profnow' AND status = 'Active' and int_courseGroup = 24 order by LName";
							...
							echo  '<h2 class="header">' . getName($EmpID) .'</h2>
								<table class="table table-bordered table-hover responsive-utilities">
								  <tbody>
									<tr>
										<td class="table-narrow" colspan="5" style="text-align: left"><a class="btn btn-mini">Preffered Subjects</a> <a class="btn btn-mini">'. $strSem .'</a> <a class="btn btn-mini pull-right">'. $sy .'</a></td>
									</tr>
									<tr>
							...
							$sql = "SELECT * FROM tbl_subjpreferences WHERE sprof = '$EmpID'";
							$query = mysqli_query($conn, $sql);
							while($row = mysqli_fetch_array($query)) {
								...
								while($row = mysqli_fetch_array($query))
								{
									...
									<table class="table table-bordered table-hover responsive-utilities">
									  ...
									  <tr>
										...
										<a class="btn btn-mini">Preffered Time</a>
										</td>
									</tr>
										<tr>
						...
						function getRoom($scode,$sy,$sem)
						{
							$sql ="SELECT * FROM tbl_subjpreferences WHERE scode ='$scode'  AND sem = '$sem' AND schoolYear = '$sy' ";
							$result = mysqli_query($conn, $sql);
							$row = mysqli_fetch_array($result);
							$room = $row['sroom'];
							return $room;
						...
						function getRoom2($scode,$sy,$sem)
						{
							$sql ="SELECT * FROM tbl_subjpreferences WHERE scode ='$scode'  AND sem = '$sem' AND schoolYear = '$sy' AND courseID = '$cID'";
							$result = mysqli_query($conn, $sql);
							$row = mysqli_fetch_array($result);
							$room = $row['sroom2'];
							return $room;
						...
						function getTime2($scode,$sy,$sem)
						{
							$sql ="SELECT * FROM tbl_subjpreferences WHERE scode ='$scode'  AND sem = '$sem' AND schoolYear = '$sy' AND courseID = '$cID'";
							$result = mysqli_query($conn, $sql);
							$row = mysqli_fetch_array($result);
