<?php
	...
	if($action=="1") {
		...
		if($newuser1==null) {
			header("Location: index.php?r=administrator/cas&msg=Blank username not allowed. Please fill up your new username. &msgType=err");
		} else if($olduser==$newuser1) {
			header("Location: index.php?r=administrator/cas&msg=New username cannot be the same with your current username.&msgType=err");
		} else {
			$same = 0;
			...
			if($same==0) {
				...
				$result=mysqli_query($conn,$sql);	
				if($result) {
					header("location: index.php?r=administrator/AccSettings&msg=Username has been changed successfully.&msgType=succ");
				} else {
					header("location: index.php?r=administrator/AccSettings&msg=Username not available. Please choose different username.&msgType=succ");
				}
			} else {
				header("Location: index.php?r=administrator/cas&msg=Username unavailable. Please try different username. &msgType=err");
			}
		}
	} else if ($action=="2") {
		...
		if($as3==null || $as4==null || $as5==null) {
			header("Location: index.php?r=administrator/cas&msg.Please fill up all the required fields. &msgType=err");
		} else {
			...
			if($as3==$opass) {
				if($as4==$as5) {
					...
					$result=mysqli_query($conn,$sql);	
					header("location: index.php?r=administrator/AccSettings&msg=Password has been changed successfully.&msgType=succ");
				} else {
					header("location: index.php?r=administrator/AccSettings&msg=New password and re-type password mismatch. Please make sure you type it correctly.&msgType=err");
				}
			} else {
				header("location: index.php?r=administrator/AccSettings&msg=Old password didn't match. Please try again.&msgType=succ");
			}
		}
	} else if ($action=="3") {
		$as6 = $_POST['as6'];
		...
		$result = mysqli_query($conn,$sql);	
		if($result) {
			header("Location: index.php?r=administrator/AccSettings&msg=Your security question and secret answer has been set successfully.&msgType=succ");
		} else {
			header("Location: index.php?r=administrator/AccSettings&msg=Your security question and secret answer has not been set.&msgType=succ");
		}
	} else if ($action=="4") {
		...
		if($count<=0) {
			...
			if($result) {
				...
				header("Location: index.php?r=administrator/AccSettings&msg=Your new Employee ID has been saved.&msgType=succ");
			} else {
				header("Location: index.php?r=administrator/AccSettings&msg=Your security new Employee ID has not been set.&msgType=err");
			}
		} else {
			header("Location: index.php?r=administrator/AccSettings&msg=Employee ID is already in used.&msgType=err");
		}
