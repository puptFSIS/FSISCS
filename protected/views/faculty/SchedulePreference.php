...
<form id="annc" name="annc" action="index.php?r=faculty/processSaveSchedPref" method="post">
...
<select name="sem" style="width: 211px; margin-top: -29px; margin-left: 23.5%;">
	<?php
		for($i=1;$i<=3;$i++) {
		$row = $i;
			if($i==3){
...
<select name="sy" style="width: 211px; margin-top: -29px; margin-left: 23.5%;">
<?php
	$currYear=date("Y")+1;
	$prevYear=date("Y");
	...
	$sql = "SELECT DISTINCT sy FROM tbl_schedpref ORDER BY sy DESC";
	$result = mysqli_query($conn, $sql);
	while($row = mysqli_fetch_array($result)) {
		if($yearString!=$row['sy'])
		{
		echo'
