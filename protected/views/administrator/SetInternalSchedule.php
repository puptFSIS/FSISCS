...
<!-- Page title -->
<title>Internal Schedule Management | Set Schedule</title>
<!-- Page icon -->
...
<h2 class=underlined-header>Set Internal Schedule 
...
<form id="annc" name="annc" action="index.php?r=administrator/processSetInternalSched&schedID1=<?php echo $_GET['schedID1']?>&schedID2=<?php echo $_GET ['schedID2']?>&prof=<?php echo $_GET['prof']?>&currID=<?php echo $_GET['CurrID'];?>&courseID=<?php echo $_GET['courseID'];?>&cyear=<?php echo $_GET['cyear'];?>&scode=<?php echo $_GET['scode'];?>&sem=<?php echo $_GET['sem'];?>&sy=<?php echo $_GET['sy'];?>&sec=<?php echo $_GET['sec'];?>&title=<?php echo $_GET['title'];?>&units=<?php echo $_GET['units'];?>&lec=<?php echo $_GET['lec'];?>&lab=<?php echo $_GET['lab'];?>" method="post">
<p style="margin-bottom: 9px;">*Day:
<select name="sday" style="width: 470px; margin-top: -28px; margin-left: 15%;">
	<?php
		$schedID = $_GET['schedID2'];
		$blank = "";
		$d1 = "M";
		$d2 = "T";
		$d3 = "W";
...
<?php
	...
	echo'
			...
			$sql="SELECT * FROM tbl_timepreferences as a inner join tbl_subjpreferences as b on a.sprof = b.sprof where b.scode = '$scode' AND a.sem = '$sem' and a.schoolYear = '$sy' ";
			$result = mysqli_query($conn,$sql);
			while($row = mysqli_fetch_array($result)) 
			{
