...
class PDF extends FPDF
{
	...
	function TrainingPrograms()
	{
		...
		$c = 110;
		for ($ctr=0; $ctr<15; $ctr++) {
			if(strlen($title[$ctr])>=58) 
			{
				$this->labelF();
				$this->SetXY(12,$b);
				$this->Cell($w[0], 4.5, substr($title[$ctr],0,58), '', 0, 'L', $f);
				$this->SetXY(12,$c);
				$this->Cell($w[0], 4.5, substr($title[$ctr],58,58), '', 0, 'L', $f);				
			} else {
				$this->labelF();
		...
		$c = 110;
		for ($ctr=0; $ctr<15; $ctr++) {
			if(strlen($conby[$ctr])>=45) 
			{
				$this->labelF();
				$this->SetXY(153,$b);
				$this->Cell($w[0], 4.5, substr($conby[$ctr],0,45), '', 0, 'L', $f);
				$this->SetXY(153,$c);
				$this->Cell($w[0], 4.5, substr($conby[$ctr],45,45), '', 0, 'L', $f);				
			}/*
	...
	function otherInformation()
	{
		...
		for ($ctr=0; $ctr<5; $ctr++) {
			...
			//miao display
			if(strlen($miao[$ctr])>45){
			$this->SetXY(153,$b);
			$this->Cell($w[2], 4.5, substr($miao[$ctr],0,45), "LR", 0, 'C', $f);
			
			$this->SetXY(153,$c);
			$this->Cell($w[2], 4.5, substr($miao[$ctr],45,45), "LBR", 0, 'C', $f);
			}
			else{
			$this->SetFont('ARIALN','',7.5);
